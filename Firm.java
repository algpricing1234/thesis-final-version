...
class Firm {
    ...
    private final MatchLowPrice matchLowPrice;         //Holds the firm's matchLowPrice algorithm.
    private final UndercutLowestPrice undercutLowestPrice;
    private final ZeroIntelligence zeroIntelligence;
    private final BestResponse bestResponse;
    private final ProportionalTitForTat proportionalTitForTat;
    ...
    Firm(SimulationRun simulationRun, int seed) {
        ...
        matchLowPrice = new MatchLowPrice(seed);
        undercutLowestPrice = new UndercutLowestPrice(seed);
        zeroIntelligence = new ZeroIntelligence(seed);
        bestResponse = new BestResponse(seed);
        proportionalTitForTat = new ProportionalTitForTat(seed);
    }
    ...
    int chooseActionMLP(){
        // Call Match Lowest Price
        return matchLowPrice.runEpisode(getActionsOfOtherFirms());
    }

    int chooseActionULP(){
        // Call Undercut Lowest Price
        return undercutLowestPrice.runEpisode(getActionsOfOtherFirms());
    }

    int chooseActionZI(){
        return zeroIntelligence.runEpisode(getActionsOfOtherFirms());
    }
    int chooseActionBR(){
        return bestResponse.runEpisode(getActionsOfOtherFirms());
    }
    int chooseActionPTFT(){return proportionalTitForTat.runEpisode(getActionsOfOtherFirms());}
    ...
    List<Double> getMeanProfitOfFirm() {
        return simulationRun.getMeanProfitOfFirm();
    }
